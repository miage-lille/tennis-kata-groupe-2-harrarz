...
describe('Tests for transition functions', () => {
  ...
  test('Given players at 0 or 15 points score kind is still POINTS', () => {
    fc.assert(
      fc.property(G.getPoints(), G.getPlayer(), ({ pointsData }, winner) => {
        fc.pre(
          pointsData.playerOne.kind !== 'THIRTY' && 
          pointsData.playerTwo.kind !== 'THIRTY'
        );
  ...
  test('Given one player at 30 and win, score kind is forty', () => {
    fc.assert(
      fc.property(G.getPoints(), G.getPlayer(), ({ pointsData }, winner) => {
        const winnerPoints = winner === 'PLAYER_ONE' ? 
          pointsData.playerOne : 
          pointsData.playerTwo;
        
        fc.pre(winnerPoints.kind === 'THIRTY');
        
        const score = scoreWhenPoint(pointsData, winner);
        expect(score.kind).toBe('FORTY');
        if (score.kind === 'FORTY') {
          expect(score.fortyData.player).toBe(winner);
        }
      })
    );
  });
